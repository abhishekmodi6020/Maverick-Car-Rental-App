...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("Create table UserDetails(FirstName text,LastName text,Gender text,MobileNo text,Email text,MavId text,StudentFaculty text,AutoClub text,Role text,UserName text primary key,Password text,Access text)");

    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("drop table if exists UserDeta");
    }
...
//checking if email exists
    ...
    public Cursor getprofiledata(String UserName){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor profiledata = db.rawQuery("select FirstName,LastName, MobileNo, Email, MavId, AutoClub, Gender, UserName, Password, Access from UserDetails where UserName=?",new String[]{UserName});
        return profiledata;
    }

    public Cursor getallusers(){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor allusers = db.rawQuery("select FirstName,LastName, MobileNo, Email, MavId, AutoClub, Gender, UserName, Password, Access from UserDetails",null);
        return allusers;
    }
    ...
    public boolean accessupdate(String UserName, String AccessValue){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("Access",AccessValue);
        db.update("UserDetails",contentValues,"UserName=?",new String[]{UserName});
        return true;
    }
